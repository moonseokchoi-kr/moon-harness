...
Stitch MCP가 없으면 아래 ASCII 방식으로 폴백한다:
...
└─────────────────────────┘
```
ASCII 폴백 시에도 각 화면에 대해 레이아웃, 주요 요소, 인터랙션을 동일하게 기술한다.
...
- **DESIGN.md**: `docs/sdd/design/DESIGN.md` — 디자인 토큰, 컬러, 타이포그래피
...
- **Stitch MCP가 사용 가능하면 반드시 시각 디자인을 생성한다** — ASCII 와이어프레임은 Stitch 불가 시에만 폴백
- UI 명세 문서에 Stitch 스크린 ID를 기록한다 — 실제 시각 설계는 Stitch가 SOT(단일 진실 공급원)
