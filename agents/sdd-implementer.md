...
- **빌드 프로파일** (build-aware TDD): 증분 빌드 명령 + 이 태스크 스코프 테스트 명령 + no-clean 규율. 워밍업은 오케스트레이터가 Phase 4 진입 시 1회 완료 — 풀빌드/clean 금지, 증분만.
...
6. **[TDD] GREEN 확인 (증분 빌드 + 스코프 테스트)** — 빌드 프로파일로 증분 빌드(build-required) 후 이 태스크 테스트가 전부 통과(GREEN)인지 확인. 풀빌드/clean/전체 스위트 금지(fast-scoped면 스코프 테스트만).
