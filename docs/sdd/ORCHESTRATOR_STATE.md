...
### 사용자 최종 결정 (2026-08-04)

1. **kompound `AGENTS.md` 예외 목록 추가 → 승인·반영 완료.** 사용자 "예외목록에 추가해줘". `AGENTS.md`의 Exceptions 절에 `wiki/sdd-spec-registry.md` 항목을 추가했다(훅이 raw→wiki 방향을 따르며 wiki 3파일만 건드린다는 점, 허용 카탈로그 연산 4종, 날짜 박힌 서술 불변, 그 밖은 추측 대신 실패 보고라는 점을 명기). **kompound에 다른 세션의 미커밋 작업 7건이 있어 커밋하지 않았다** — 사용자가 자기 작업과 함께 커밋한다.
2. **`__init__.py` 성능 후속 → 보류.** 사용자 "성능은 실제 써본다음에 해결해보자". 별도 사이클을 지금 만들지 않고, **실사용 후 체감 여부를 보고 판단**한다. 실측치(+43.5ms/Stop 훅, arch 목표 20ms의 2배)와 수정 경로(PEP 562 지연 `__getattr__`)는 아래에 그대로 보존한다.
3. **이 사이클 자신의 문서 박제 → 머지 후 실행.** 사용자 동의. 현재 코드 미머지 + kompound dirty(미커밋 7건)로 F9가 차단하는 상태이므로 순서상 머지 후가 맞다.

### 🟡 후속 태스크 (이 사이클 범위 밖 — **사용자 보류 결정: 실사용 후 판단**)
