...
| 모델 | 용도 | 비용 비율 |
|------|------|----------|
| **opus** | 아키텍처 결정, 설계 판단, 근본적 비판 — 추론력이 필요한 작업 | 10x |
| **sonnet** | 코드 구현, 리뷰, 테스트 — 90%의 실제 작업 | 3x |
| **haiku** | 패턴 매칭, 검색 정리, 단순 판단, 훅 prompt — 경량 작업 | 1x |

전부 opus로 쓰면 비용만 10배, 품질 향상 없음. sonnet이 구현/리뷰의 주력이고, opus는 설계 판단에만 집중.
...
각 태스크 시작/완료 시 cmux 진행률을 업데이트한다 (cmux 없으면 무시):

```bash
# 태스크 시작 시 (N: 현재 태스크 번호, M: 전체 태스크 수, TASK_NAME: 태스크 이름)
cmux set-progress $(echo "scale=2; ($N - 1) / $M" | bc) --label "T-$N/$M: $TASK_NAME" 2>/dev/null || true
cmux set-status sdd "T-$N/$M" --icon "hammer" 2>/dev/null || true

# 태스크 완료 시
cmux set-progress $(echo "scale=2; $N / $M" | bc) --label "T-$N/$M 완료" 2>/dev/null || true

# 모든 태스크 완료 시
cmux set-progress 1.0 --label "SDD: 구현 완료" 2>/dev/null || true
