...
### Step 2b: `.harness/` 디렉터리 + LEARNING.md 초기화

`.harness/`는 에이전트가 자동으로 누적하는 산출물 영역이다. `docs/`(사람이 큐레이팅하는 문서)와 구분된다.
...
SDD 에이전트(sdd-implementer, sdd-reviewer, sdd-compliance-checker 등)가
사이클 중 발견한 반복 가능한 교훈을 append한다. 사람이 직접 편집하지 않는다.

거르고 일반화된 교훈은 docs/lessons-learned.md로 승격한다.
...
> **타깃 결정 (CLAUDE.md 부재 시 폴백)**: 이 Step의 자동 업데이트/포인터 주입은 **타깃 파일**을 먼저 정한다.
> - `<repo>/CLAUDE.md`가 **있으면** → 그 파일을 타깃으로 한다(아래 그대로).
> - `<repo>/CLAUDE.md`가 **없으면** → committed CLAUDE.md를 강제 생성하지 말고 **repo 루트 `./CLAUDE.local.md`를 타깃**으로 한다(없으면 생성). 공유 레포에선 committed CLAUDE.md를 원치 않을 수 있고, `CLAUDE.local.md`는 로컬/gitignore 관행이기 때문(빌드 프로파일 발견의 `./CLAUDE.local.md` 폴백과 동일 원칙). `.gitignore`에 `CLAUDE.local.md`가 없으면 추가 제안.
> - 아래 "CLAUDE.md"로 표기된 모든 동작(목차화 제안, 학습 포인터 주입, import 라인 추가)은 이 타깃 파일에 적용한다.
...
`@.harness/LEARNING.md` 줄은 Claude Code의 import 문법으로, 파일 내용 전체를 컨텍스트에 자동 포함시킨다. SDD 에이전트가 append한 raw 교훈이 다음 사이클 시작 시 자동 로드된다.

**타깃 파일(CLAUDE.md 또는 폴백 시 CLAUDE.local.md)에 import 라인이 없으면 추가만 한다** (다른 내용은 건드리지 않음). 타깃이 CLAUDE.local.md면 `@.harness/LEARNING.md` 등 포인터를 거기에 주입한다.
