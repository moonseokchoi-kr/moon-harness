...
기능 아이디어를 **Spec(요구 명세) → Design(아키텍처·IA·UI·API)** 2단계로 구조화하여 구현 직전까지의 설계 산출물을 생산하는 워크플로우.
...
└── e2e-config.json                      # Phase 2 완료 시 ui-designer가 생성
...
| `ui-designer` | 2 | opus | UI/UX 명세 |
| `blocker-checker` | 1 | haiku | spec 블로커 탐지 |
| `context-manager` | 전체 | haiku | 공유 상태 관리 |
...
3. ui-designer → 순수 UX 명세 (아키텍처 제약 안에서)
   ...
   → 산출물: design/ui/ + docs/spec-design/design/DESIGN.md (둘 다 필수)
   → architect-reviewer → UI 명세 리뷰 → 수정 → PASS
   → [사용자 UI 피드백] → 반영
   → PHASE2_UI_DESIGN_COMPLETE

4. api-designer (UI 명세 필수 입력) → UI 명세에서 도출된 데이터 요구사항 + arch 기반 API/데이터 계약 설계
...
6. context-manager (FULL 모드) → context 문서 초기 생성
...
| 2 | `docs/spec-design/design/arch/{YYYY-MM-DD}-{feature}.md` 존재 | **Phase 2로 롤백** — architect 재디스패치 |
| 3 | FULL 모드인 경우 `docs/spec-design/design/ui/` + `docs/spec-design/design/api/` 존재 | **Phase 2로 롤백** — 누락분 복원 |
...
| 2 | `docs/spec-design/task/{feature}/` 아래 1개 이상의 task 문서 | **Phase 3으로 롤백** — taskmaster 재디스패치 |
