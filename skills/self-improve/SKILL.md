...
> 📌 Phase B에서 **집계 로더로 합쳐진 entries**를 `count_signals()`에 전달하여 교차 프로젝트 여부를 확인한다. store 미설정 시 로컬 entries만 반영(하위호환). `has_cross_project=True` 이어야만 하네스 티어로 에스컬레이션 경로를 열 수 있다.
